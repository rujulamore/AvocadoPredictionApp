...
def evaluate_valset():
    ...
    fpr, tpr, _ = roc_curve(labels_list, prob_list)
    plt.figure()
    plt.plot(fpr, tpr, label='ROC curve (area = {:.2f})'.format(roc_auc_score(labels_list, prob_list)))
    plt.plot([0, 1], [0, 1], 'k--')
    plt.xlabel('False Positive Rate')
    plt.ylabel('True Positive Rate')
    plt.title('ROC Curve')
...
model = models.efficientnet_b0(pretrained=True)  # Use the correct EfficientNet variant
model.classifier[1] = nn.Linear(model.classifier[1].in_features, 2)  # Replace final layer
...
for epoch in range(1, EPOCHS+1):
    ...
    if log_test['val_accuracy'] > best_test_accuracy: 
        old_best_checkpoint_path = 'efficientnet_64best-acc-{:.3f}.pth'.format(best_test_accuracy)
        if os.path.exists(old_best_checkpoint_path):
            os.remove(old_best_checkpoint_path)

        best_test_accuracy = log_test['val_accuracy']
        new_best_checkpoint_path = 'efficientnet_64best-acc-{:.3f}.pth'.format(log_test['val_accuracy'])
    ...
    #     old_best_checkpoint_path = 'efficientnet_64best-{:.3f}.pth'.format(best_test_accuracy)
    #     if os.path.exists(old_best_checkpoint_path):
    #         os.remove(old_best_checkpoint_path)

    #     best_test_accuracy = log_test['val_accuracy']
    #     new_best_checkpoint_path = 'efficientnet_64best-{:.3f}.pth'.format(log_test['val_accuracy'])
    #     torch.save(model, new_best_checkpoint_path)
    #     print('saving the optimal model', 'efficientnet_64best-{:.3f}.pth'.format(best_test_accuracy))
    #     # best_test_accuracy = log_test['test_accuracy']

df_train_log.to_csv('efficientnet_64trainlog.csv', index=False)
df_val_log.to_csv('efficientnet_64validationlog.csv', index=False)

# Load the best model based on validation accuracy before testing
# best_model_path = 'efficientnet_64best-{:.3f}.pth'.format(best_test_accuracy)
...
best_model_path = 'efficientnet_64best-acc-{:.3f}.pth'.format(best_test_accuracy)

# ✅ Re-instantiate the model and load weights
model = models.efficientnet_b0(pretrained=False)  # No need to redownload pretrained weights
...
# Manually re-evaluate for plots and save
labels_list = []
preds_list = []
prob_list = []
...
weight_path = os.path.join(output_dir, "classifier_weights.csv")
...
# Force same y-axis range across all models for visual symmetry
plt.ylim(0.0, 1.75)
