...
mobilenet = models.efficientnet_b0(pretrained=True)
num_ftrs = mobilenet.classifier[1].in_features
mobilenet.classifier[1] = nn.Linear(num_ftrs, 1)  # Regression output
...
for epoch in range(num_epochs):
    ...
    if val_loss < best_val_loss:
        ...
        model_pkl_path = f'RegressionWeights/{batch}_Best-efficientnet_{num_epochs}.pth'  # Use .pth extension for PyTorch model
...
plt.savefig(f'LossFunction/Efiicientnet_{date_time}_{batch}_{num_epochs}.png')
plt.close()
...
loss_csv_path = f'LossFunction/Losses_efficientnet_{date_time}_{batch}_{num_epochs}.csv'
...
plt.title(f'Predicted vs Actual (R² = {r2:.4f})', fontsize=16)
plt.legend(fontsize=12)
plt.grid(True)
plt.tick_params(axis='both', labelsize=12)


pred_vs_actual_path = f'graphs/Predicted_vs_Actual_efficientnet{date_time}_{batch}_{num_epochs}.png'
...
plt.title(f'Test Predicted vs Actual (R² = {test_r2:.4f})', fontsize=16)
plt.legend(fontsize=12)
plt.grid(True)
plt.tick_params(axis='both', labelsize=12)
...
test_pred_vs_actual_path = f'graphs/Test_Predicted_vs_Actual_efficientNet_{date_time}_{batch}_{num_epochs}.png'
...
final_fc_weights = best_model.classifier[1].weight.detach().cpu().numpy().flatten()

# Plot the weights as a bar graph
# plt.figure(figsize=(12, 6))
# plt.bar(range(len(final_fc_weights)), final_fc_weights, color='orange')
# plt.xlabel('Feature Index')
# plt.ylabel('Weight Value')
# plt.title('Final Fully Connected Layer Weights (EfficientNet)')
# plt.grid(True)
plt.figure(figsize=(12, 6))
plt.bar(range(len(final_fc_weights)), final_fc_weights, color='orange')
plt.xlabel('Feature Index', fontsize=14)
plt.ylabel('Weight Value', fontsize=14)
plt.title('Final Fully Connected Layer Weights (EfficientNet)', fontsize=16)
plt.grid(True)
plt.tick_params(axis='both', labelsize=12)


# Save the weight plot
weights_plot_path = f'graphs/FC_Weights_BarGraph_efficientNet_{date_time}_{batch}_{num_epochs}.png'
plt.savefig(weights_plot_path)
plt.close()

print(f"Fully connected layer weights bar graph saved to {weights_plot_path}")


# Save R² and RMSE to a text file
metrics_path = f'graphs/Test_Metrics_EfficientNet_{date_time}_{batch}_{num_epochs}.txt'
